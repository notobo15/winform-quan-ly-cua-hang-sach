...
{
    public partial class FRpOrder : Form
    {
        ...
        public FRpOrder(string orderId)
        {
            InitializeComponent();
            OnlyPrint = true;
            this.orderId = orderId;
        }
        public FRpOrder(int CustomerId)
        {
            InitializeComponent();
            OnlyPrint = false;
        ...
        private void FRpOrder_Load(object sender, EventArgs e)
        {
            ...
            reportViewer1.LocalReport.ReportPath = "C:\\Users\\notobo\\source\\repos\\BT C#\\WindowsFormsApp3\\WindowsFormsApp3\\Gui\\Report\\RpOrder.rdlc";

            CustomOrder CustomOrder = null;
            if (OnlyPrint)
            {
                btnOrderConfirm.Enabled = false;
                var order = OrderBus.getFirstById(orderId);
                Carts = OrderBus.getListOrder(orderId);
                ...
                LastId = OrderBus.CreateOrder(new Order()
                {
        ...
        private void materialButton1_Click(object sender, EventArgs e)
        {
            ...
            if (mess == DialogResult.OK)
            {
                ...
                {
                    OrderDetailBus.Add(new OrderDetail() { 
                ...
                foreach (var item in Carts)
                {
                    BookDetailBus.UpdateQuantitySell(item.BookDetailId, item.Quantity);
                }
        ...
        {
            OrderBus.DeleteHardById(LastId);
        }
    }
}
